...
class DumpTextParser:
    ...
    def Analyze(self):
        ...
        #res+= "Process Name = %s\n" % self.process_name
        res.append(("faulting_ip_call",self.faulting_ip_call))
        res.append(("faulting_ip_assembly_instruction",self.faulting_ip_assembly_instruction))
        #res+="faulting_thread = 0x%X\n" % (self.faulting_thread_ID)
        res.append(("process uptime",self.process_uptime))
        res.append(("system uptime",self.system_uptime))
        
        #print "exception stack:"
        #for el in self.stack_text:
        #    print el[:-1]
                       
        res.append(("exception code",self.exception_code))
        res.append(("exception code description",self.exception_code_str))
        ...
        faulting_thread_stack = ""        
        if self.faulting_thread_ID in self.all_threads_map:
            for el in self.all_threads_map[self.faulting_thread_ID]:
                faulting_thread_stack+=el
                faulting_thread_stack+="\n"        
        else:
            faulting_thread_stack+="Unidentified faulting thread - [%s]" % self.faulting_thread_ID
            faulting_thread_stack+='\n'            
            

        res.append(("faulting thread stack:",faulting_thread_stack))
        return res
    ...
    def ReadFile(self,input_log_file):
        ...
        #print Threads_Times[0]
        #if Threads_Times[0].find('No export') != -1:
#            pass
#        else:
