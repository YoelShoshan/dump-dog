// ExceptionHandler.cpp : Defines the entry point for the console application.
//
...
bool SpawnDumperProcess(_EXCEPTION_POINTERS *pep)
{
	...
	wsprintf(processId,L"%d",dwID);

	std::wstring exe_path(L"C:/CrashPlayground/l_WatchDog.exe");
...
LONG WINAPI LucidUnhandledExceptionFilter(
...
{
	//SpawnDumperProcess(NULL);

	printf("Starting ExceptionHandler...\n");

	SetUnhandledExceptionFilter(LucidUnhandledExceptionFilter);

	
	printf("Before faulty write\n");

	int* pI = (int*) 0x0;
	*pI = 15;
	
	printf("After faulty write\n");

	//__asm int 3;	
	//DebugBreak();

	printf("Before division by zero\n");
